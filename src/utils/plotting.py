...
def price_chart(rt_df: pd.DataFrame, da_df: pd.DataFrame):
    ...
    fig.update_layout(height=360, margin={"l": 10, "r": 10, "t": 30, "b": 10}, yaxis_title="$/MWh", legend_title="")
    return fig
...
def load_chart(load_df: pd.DataFrame):
    ...
    return px.line(load_df, x="timestamp", y="load_mw", labels={"load_mw": "MW", "timestamp": "Time"}).update_layout(
        height=300,
...
def renewable_chart(wind_df: pd.DataFrame, solar_df: pd.DataFrame):
    ...
    fig.update_layout(height=300, margin={"l": 10, "r": 10, "t": 30, "b": 10}, yaxis_title="MW", legend_title="")
    return fig
